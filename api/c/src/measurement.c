...
double measurement_concentrationWithKit(const Measurement_t * self, const Factors_t * factors, const Kit_t * kit)
{
    ...
    double rfu = measurement_rfu(self, factors);
    return kit_apply(kit, interpolated, rfu, factors->stdLow.concentration, factors->stdHigh.concentration, factors->stdHigh.value);
}
